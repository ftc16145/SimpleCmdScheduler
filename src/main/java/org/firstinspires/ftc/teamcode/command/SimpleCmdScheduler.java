 ...
 * SimpleCmdScheduler#run()} on the singleton instance in its periodic block in order to run commands
 * synchronously from the main loop.  Subsystems should be registered with the scheduler using
 * {@link SimpleCmdScheduler#registerSubsystem(Subsystem...)} in order for their {@link
 * Subsystem#periodic()} methods to be called and for their default commands to be scheduled.
 */

public final class SimpleCmdScheduler {
  /**
   * The Singleton Instance.
   */
  private static SimpleCmdScheduler instance;
   ...
   * @return the instance
   */
  public static synchronized SimpleCmdScheduler getInstance() {
    if (instance == null) {
      instance = new SimpleCmdScheduler();
    }
    return instance;
  }
  ...
  private boolean m_robotDisabled=false;
  public void setRobotDisabled( boolean disabled ) {
    m_robotDisabled = disabled;
  }
  ...
  // Public only to support testing
  public SimpleCmdScheduler() {
  }
  ...
  private void schedule(boolean interruptible, Command command) {
    ...
    if (m_disabled || (m_robotDisabled && !command.runsWhenDisabled())
  ...
  public void run() {
    ...
    for (Iterator<Command> iterator = m_scheduledCommands.keySet().iterator();
      ...
      if (!command.runsWhenDisabled() && m_robotDisabled ) {
        command.end(true);
  ...
  public void unregisterSubsystem(Subsystem... subsystems) {
    m_subsystems.keySet().removeAll( new HashSet<>( Arrays.asList(subsystems)) );
  }
  ...
  public boolean isScheduled(Command... commands) {
    return m_scheduledCommands.keySet().containsAll( new HashSet<>( Arrays.asList(commands)) );
  }
